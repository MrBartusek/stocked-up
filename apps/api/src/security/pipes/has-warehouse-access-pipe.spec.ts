...
import { HasWarehouseAccessPipe } from './has-warehouse-access.pipe';

const VALID_ORG_ID = new Types.ObjectId();
const VALID_ENTITY_ID = new Types.ObjectId();

describe('HasWarehouseAccessPipe', () => {
	let pipe: HasWarehouseAccessPipe;

	beforeEach(async () => {
		const module: TestingModule = await Test.createTestingModule({
			providers: [
				HasWarehouseAccessPipe,
				{
					provide: REQUEST,
		...
		pipe = await module.resolve<HasWarehouseAccessPipe>(HasWarehouseAccessPipe);
	});
