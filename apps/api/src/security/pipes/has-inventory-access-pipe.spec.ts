...
import { HasInventoryAccessPipe } from './has-inventory-access.pipe';

const VALID_ORG_ID = new Types.ObjectId();
const VALID_ENTITY_ID = new Types.ObjectId();

describe('HasInventoryAccessPipe', () => {
	let pipe: HasInventoryAccessPipe;

	beforeEach(async () => {
		const module: TestingModule = await Test.createTestingModule({
			providers: [
				HasInventoryAccessPipe,
				{
					provide: REQUEST,
		...
		pipe = await module.resolve<HasInventoryAccessPipe>(HasInventoryAccessPipe);
	});
